...
## Coverage-only hard-block hotfix (branch `fix-coverage-only-hard-block`, 2026-09-07)

Fresh production case (2-lot) blocked both lots REPORT_BLOCKED despite internally-READY reports (blocking_issues=0, critical_omissions=0, APPROVE_WITH_WARNINGS). Root cause: `coverage_audit._state` returned FAIL on `ratio<0.60` even with `critical_missing=False` (ratio depressed by legit subordinate/detail "parziale" money rows) → `coverage_status=FAIL` → `quality_gate` hard-fail → empty blocking codes → `partial_report` fail-closed BLOCKED. Fixed at the producers (non-critical low ratio → WARNING; gate FAIL from structured facts; unexplained coverage FAIL fails closed + diagnostic). Disclosure: READY_WITH_WARNINGS / FULL_REPORT_AVAILABLE + `coverage_incomplete=true` (owner-approved 2026-09-07). partial_report allow-list unchanged (no empty-code bypass). Fable APPROVE (10 attacks defended, reproduced).

| ID | Sev | Risk | Status | Control / note |
|----|-----|------|--------|----------------|
| RH-01 | S1 | Aggregate coverage ratio alone hard-blocked a materially-complete customer report (fresh production over-block) | CLOSED (pending deploy) | `_state` FAIL reserved for `critical_missing`; `quality_gate` FAIL from structured safety facts; unexplained coverage FAIL fails closed + diagnostic; 14-test suite incl. hard-block matrix + REPORT_BLOCKED⟹explicit-reason invariant + multi-lot isolation. Fable APPROVE. |
| RH-02 | S3 | `coverage_incomplete`/`coverage_incomplete_reason` marker is admin/telemetry-only — not surfaced in the customer payload | OPEN (owner-gated follow-up) | Fable MINOR. Customer is already kept off clean-green by pre-existing `pronto_con_avvertenze` (PASS_WITH_WARNINGS) + report-clarity verification items, so no safety gap. Surfacing the specific coverage-incomplete note into the customer payload thematically belongs with report-clarity — owner decides whether to wire it. |
| RH-03 | S3 | "coverage FAIL ⟹ explained" invariant enforced log-only/fail-closed, not a hard raise | OPEN (accepted, monitored) | Intentional: a prod raise would be worse than a safe block. A future unexplained coverage FAIL still fails closed (BLOCKED). Covered by `test_coverage_fail_invariant_holds_across_fixture_shapes`; a stronger CI assertion is a possible follow-up. |
| RH-04 | S3 | Report-clarity `.env` enablement leaked into the test suite (3 correctness_v2 tests failed in full run, passed in isolation) | CLOSED | `backend/conftest.py` `TEST_FEATURE_FLAG_DEFAULTS` now pins `CORRECTNESS_V2_REPORT_CLARITY_ENABLED=false`; full backend deterministic at 7 known-stale. |
