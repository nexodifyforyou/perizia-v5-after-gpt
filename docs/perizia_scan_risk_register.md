...
## Branch 6 — report clarity + Italian-first bilingual (IN PROGRESS, branch `feature-correctness-v2-report-clarity`)

Presentation/IA + bilingual layer over Branches 1–3; flag `CORRECTNESS_V2_REPORT_CLARITY_ENABLED` (default OFF). No semantic re-derivation; English is translation-only and never feeds severity/status/readiness/disclosure.

| ID | Sev | Risk | Status | Control / note |
|----|-----|------|--------|----------------|
| R6-01 | S2 | Material CanonicalVerdict conflict classes may deserve to influence canonical severity/readiness/esito, not just presentation | **OPEN (follow-up, owner 2026-08-15)** | Fable finding #3. This UX branch applies a **presentation-only** amber floor: REPORT_READY + non-empty `canonical_verdict.conflicts` never shows an unconditional green "Nessuna verifica bloccante"; instead a deterministic qualified bilingual summary ("Nessun blocco automatico rilevato, ma sono presenti informazioni in conflitto da verificare." / "No automatic blocking issue was detected, but conflicting information requires verification.") with a ≥amber displayed tone. **CanonicalVerdict/esito/severity/readiness/REPORT_READY are NOT changed.** Whether specific material conflict classes should change canonical severity/esito is a separate SEMANTIC-quality decision to review later — do NOT change that policy inside this UX branch. |
