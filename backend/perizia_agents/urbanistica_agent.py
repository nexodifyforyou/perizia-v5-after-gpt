...
_FIELD_GUARDS = {
    "urbanistica_status": [
        "scoped_urbanistica_resolution",
        "weak_urbanistica_wording_stays_non_verificabile",
        "root_urbanistica_leaf_first_rollup",
    ],
    "sanatoria_status": [
        "scoped_sanatoria_resolution",
        "historical_sanatoria_mentions_not_auto_truth",
        "root_urbanistica_leaf_first_rollup",
    ],
    "condono_status": [
        "scoped_condono_resolution",
        "historical_condono_mentions_not_auto_truth",
        "root_urbanistica_leaf_first_rollup",
    ],
    "ripristino_or_demolition_signal": [
        "scoped_ripristino_resolution",
        "remedial_signal_requires_current_prescriptive_wording",
        "root_urbanistica_leaf_first_rollup",
    ],
}

_URBANISTICA_POSITIVE_RE = re.compile(
    r"\b(?:"
    r"immobile\s+risulta\s+regolare|"
    r"risulta\s+regolare\s+per\s+la\s+legge\s+n?[°o]?\s*47/1985|"
    r"(?:immobile|unit[aà]|unit[aà] immobiliari|beni?)\b[^\n]{0,90}\bconform\w+\s+a\s+quanto\s+depositato|"
    r"non\s+(?:risultano|emergono)\s+(?:abusi|difformit\w+|irregolarit\w+)|"
    r"assenza\s+di\s+(?:abusi|difformit\w+|irregolarit\w+)|"
    r"conforme\s+urbanistic\w*|"
    r"regolare\s+urbanistic\w*|"
    r"compatibil\w+[^\n]{0,80}strumenti\s+urbanistic\w*|"
    r"risultan\w+\s+regolar\w+\s+urbanistic\w*"
    r")\b",
    re.IGNORECASE,
)
_URBANISTICA_NEGATIVE_RE = re.compile(
    ...
    r"opere?\s+abusive?\w*|"
    r"non\s+conform\w+|"
    r"modifiche?\s+eseguite\s+senza\s+pratiche\s+edilizie"
    r")\b",
    re.IGNORECASE,
)
...
_CONDONO_ASSENTE_RE = re.compile(
    r"\b(?:"
    r"non\s+(?:risultano|sono\s+presenti|e\s+presente)\s+(?:domande?\s+di\s+)?condon\w+|"
    r"assenza\s+di\s+condon\w+|"
    r"non\s+risulta\s+alcun\s+condon\w+"
    r")\b",
    re.IGNORECASE,
)
_RIPRISTINO_NO_RE = re.compile(
    r"\b(?:"
    r"non\s+si\s+rende\s+necessari\w*[^\n]{0,40}(?:ripristin\w+|demolizion\w+)|"
    r"non\s+sono\s+necessari[^\n]{0,40}(?:ripristin\w+|demolizion\w+)|"
    r"nessuna\s+demolizion\w+\s+necessari\w*"
    r")\b",
    re.IGNORECASE,
)
_RIPRISTINO_YES_RE = re.compile(
    r"\b(?:"
    r"si\s+rende\s+necessari\w*[^\n]{0,40}(?:ripristin\w+|demolizion\w+)|"
    r"dovr[aà]\s+(?:essere\s+)?(?:ripristinat\w+|demolit\w+)|"
    r"rimess\w+\s+in\s+pristin\w+|"
    r"demolizion\w+\s+(?:delle|degli|dei)\s+opere\s+abusive|"
    r"opere\s+da\s+demolire"
    r")\b",
    re.IGNORECASE,
)
...
def _line_context_quote(
    *,
    line: str,
    active_lotto_line: str | None,
    active_bene_line: str | None,
    include_active_bene_line: bool = True,
) -> str:
    parts: list[str] = []
    if active_lotto_line:
        parts.append(active_lotto_line.strip())
    if include_active_bene_line and active_bene_line and active_bene_line != active_lotto_line:
        parts.append(active_bene_line.strip())
...
def _verification_trail(
    ...
    *,
    ...
    return {
        ...
        "verify_next": [
            "Verifica titoli edilizi e accesso agli atti comunali.",
            "Conferma se la frase si riferisce a tutto il bene o solo a una porzione.",
        ],
    }


def _detect_urbanistica_status(line: str) -> tuple[str, int, float, str] | None:
    has_positive = bool(_URBANISTICA_POSITIVE_RE.search(line))
    has_negative = bool(_URBANISTICA_NEGATIVE_RE.search(line))
    ambiguous = bool(_AMBIGUOUS_RE.search(line))

    if has_negative and not ambiguous:
        return "DIFFORMITA_PRESENTE", 3, 0.93, "explicit_difformita"
    if has_positive and not has_negative and not ambiguous:
        return "REGOLARE", 2, 0.89, "explicit_regolare"
    if has_positive and has_negative:
        return "NON_VERIFICABILE", 1, 0.55, "same_line_mixed_signal"
    return None
...
def _detect_sanatoria_status(line: str) -> tuple[str, int, float, str] | None:
    ...
    if _SANATORIA_POSITIVE_RE.search(line) and not ambiguous:
        return "SANABILE", 2, 0.91, "explicit_sanabile"
    return None


def _detect_condono_status(line: str) -> tuple[str, int, float, str] | None:
    ambiguous = bool(_AMBIGUOUS_RE.search(line))
    if _CONDONO_ASSENTE_RE.search(line) and not ambiguous:
        return "ASSENTE", 2, 0.88, "explicit_condono_assente"
    if _CONDONO_PRESENTE_RE.search(line) and not ambiguous:
        return "PRESENTE", 2, 0.88, "explicit_condono_presente"
    return None


def _detect_ripristino_signal(line: str) -> tuple[str, int, float, str] | None:
    if _RIPRISTINO_NO_RE.search(line):
        return "NO", 2, 0.86, "explicit_no_ripristino"
    if _RIPRISTINO_YES_RE.search(line):
        return "YES", 2, 0.9, "explicit_yes_ripristino"
    return None


def _collect_hits(state: RuntimeState) -> dict[str, list[dict[str, Any]]]:
    collected: dict[str, list[dict[str, Any]]] = defaultdict(list)
    hit_index = 0
    state.candidates["urbanistica_status"] = []
    state.candidates["sanatoria_status"] = []
    state.candidates["condono_status"] = []
    state.candidates["ripristino_or_demolition_signal"] = []
        ...
        for raw_line in text.splitlines():
            ...
            for field_key, detection in detections.items():
                ...
                ownership = state.attach_evidence_ownership(
                    scope_id=scope_id,
                    field_target=f"urbanistica.{field_key}",
...
def _root_rollup_payload(
    ...
    *,
    ...
    if leaf_scope_ids == ["document_root"]:
        direct_root = state.scopes["document_root"].urbanistica.get(field_key)
    ...
    for scope_id in leaf_scope_ids:
        payload = state.scopes.get(scope_id).urbanistica.get(field_key) if state.scopes.get(scope_id) else None
    ...
    if not decisive:
        ...
        payload = _field_payload(
            ...
            verification_trail={
                ...
                "verify_next": [
                    "Verifica regolarita edilizia e accesso agli atti comunali per ogni bene.",
                ],
            },
        )
    ...
    if len(decisive_values) > 1:
        ...
        payload = _field_payload(
            ...
            verification_trail={
                ...
                "verify_next": [
                    "Verifica la regolarita edilizia separatamente per ciascun bene.",
                ],
            },
        )
        return payload, internal
    ...
    if missing_scope_ids:
        ...
        payload = _field_payload(
            ...
            verification_trail={
                ...
                "verify_next": [
                    "Verifica i beni senza evidenza urbanistica esplicita.",
                ],
            },
        )
        return payload, internal
...
def run_urbanistica_agent(state: RuntimeState) -> None:
    ...
    for field_key, direct_hits in collected.items():
        ...
        for scope_id, hits in hits_by_scope.items():
            ...
            if payload:
                state.scopes[scope_id].urbanistica[field_key] = payload
