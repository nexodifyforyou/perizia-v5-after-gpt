"""
Regression runner for valuation_candidate_pack.
...
def main() -> None:
    ...
    for case in load_cases():
        out = build_valuation_candidate_pack(case.case_key)
        ...
        rows.append({
            ...
            "artifact": (
                ...
                f"{case.case_key}/artifacts/valuation_candidate_pack.json"
            ),
        })

    fp = Path(
        "/srv/perizia/_qa/canonical_pipeline/valuation_candidate_pack_regression_summary.json"
    )
    fp.write_text(json.dumps(rows, ensure_ascii=False, indent=2), encoding="utf-8")
    print(fp)
    print(f"COUNT={len(rows)}")
    for r in rows:
        fields = r["coverage"].get("valuation_fields_present", [])
        scope_keys = r["coverage"].get("valuation_scope_keys", [])
        print(
            f"  {r['case_key']}: status={r['status']} "
            f"active={r['active_candidate_count']} "
            f"blocked={r['blocked_or_ambiguous_count']} "
            f"fields={fields} scope_keys={scope_keys}"
        )
