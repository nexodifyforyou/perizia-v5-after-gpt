...
const AdminAssistant = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Errore nel caricamento assistant');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <AdminLayout title="Assistant QA" subtitle="Domande assistente (lightweight)">
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <Input
            placeholder="Search case_id, run_id, question"
            value={q}
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.qa_id} className="border-b border-zinc-800 text-zinc-200">
                    <td className="py-2">{item.qa_id}</td>
                    <td className="py-2">{item.email || '-'}</td>
                    <td className="py-2">{item.case_id || '-'}</td>
                    <td className="py-2">{item.run_id || '-'}</td>
                    <td className="py-2">{item.question_preview || '-'}</td>
                    <td className="py-2">{item.created_at ? new Date(item.created_at).toLocaleString() : '-'}</td>
                  </tr>
                ))}
                {items.length === 0 && (
                  <tr>
                    <td colSpan="6" className="py-6 text-center text-zinc-500">Nessuna domanda</td>
                  </tr>
                )}
              </tbody>
            </table>
