...
const AdminPerizie = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${API_URL}/api/admin/perizie`, {
        params: {
          q: q || undefined,
      ...
      toast.error('Errore nel caricamento perizie');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AdminLayout title="Perizie" subtitle="Tutte le analisi perizie (lightweight)">
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <Input
            placeholder="Search case_id, run_id, analysis_id"
            value={q}
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.analysis_id} className="border-b border-zinc-800 text-zinc-200">
                    <td className="py-2">{item.analysis_id}</td>
                    <td className="py-2">{item.email || '-'}</td>
                    <td className="py-2">{item.case_id}</td>
                    <td className="py-2">{item.run_id}</td>
                    <td className="py-2">{item.revision ?? '-'}</td>
                    <td className="py-2">{item.semaforo || '-'}</td>
                ...
                {items.length === 0 && (
                  <tr>
                    <td colSpan="8" className="py-6 text-center text-zinc-500">Nessuna perizia</td>
                  </tr>
                )}
              </tbody>
            </table>
