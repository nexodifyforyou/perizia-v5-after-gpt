...
const Sidebar = ({ user, logout }) => {
  ...
  const renderNavItems = (items, { compact = false } = {}) =>
    items.map((item) => {
      const isActive =
        ...
        <Link
          ...
          className={`flex items-center gap-3 rounded-lg text-sm font-medium transition-all ${
            ...
            isActive
              ...
              : item.disabled
                ? 'cursor-not-allowed bg-zinc-950/70 text-zinc-600'
  ...
  const renderSidebarBody = ({ compact = false } = {}) => (
    <>
      <div className={`${compact ? 'px-4 py-4' : 'p-6'} border-b border-zinc-800`}>
      ...
      <nav className={`${compact ? 'p-3' : 'p-4'} flex-1 space-y-1 overflow-y-auto`}>
        {renderNavItems(navItems, { compact })}
        {user?.is_master_admin && (
          <div className="pt-6">
            <p className="mb-2 px-4 text-xs font-mono uppercase tracking-wider text-zinc-500">ADMIN</p>
            {renderNavItems(adminItems, { compact })}
          </div>
        )}
      </nav>
  ...
  return (
    <>
      <div className="fixed inset-x-0 top-0 z-40 border-b border-zinc-800 bg-[#09090b]/95 backdrop-blur lg:hidden">
        <div className="flex items-center justify-between gap-3 px-4 py-3">
          <Link to="/dashboard" className="flex min-w-0 items-center gap-3">
            <Scale className="h-7 w-7 flex-shrink-0 text-gold" />
            <div className="min-w-0">
              <p className="truncate text-lg font-serif font-bold text-zinc-100">Nexodify</p>
              <p className="truncate text-[10px] font-mono uppercase tracking-[0.24em] text-zinc-500">PeriziaScan</p>
            </div>
          </Link>
          <Sheet>
            <SheetTrigger asChild>
              <Button variant="outline" size="icon" className="border-zinc-700 bg-zinc-950 text-zinc-100 hover:bg-zinc-900">
                <Menu className="h-5 w-5" />
              </Button>
            </SheetTrigger>
            <SheetContent side="left" className="flex w-[min(88vw,22rem)] flex-col border-zinc-800 bg-zinc-950 p-0 text-zinc-100">
              <SheetHeader className="sr-only">
                <SheetTitle>Navigazione</SheetTitle>
                <SheetDescription>Menu principale PeriziaScan</SheetDescription>
              </SheetHeader>
              {renderSidebarBody({ compact: true })}
            </SheetContent>
          </Sheet>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#09090b]">
      ...
      <main className="px-4 pb-8 pt-24 sm:px-6 lg:ml-64 lg:px-8 lg:pt-8">
              ...
              stats.recent_analyses.map((analysis) => (
                ...
                >
                  <div className="flex min-w-0 items-center gap-4">
                    ...
                    <div className="min-w-0">
                      <p className="truncate text-sm font-medium text-zinc-100">{analysis.case_title || analysis.case_id}</p>
        ...
        {availablePeriziaCredits <= 1 && accountState.planId === 'free' && (
          <div className="mt-8 flex flex-col gap-4 rounded-xl border border-amber-500/30 bg-amber-500/10 p-6 sm:flex-row sm:items-center">
